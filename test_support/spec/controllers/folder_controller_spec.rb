...
describe FolderController do
  ...
  it "should add items to list" do
    ...
    Bookmark.find_all_by_session_id(request.session_options[:id]).length.should == 1
    get :update, :id => "94120425"
    Bookmark.find_all_by_session_id(request.session_options[:id]).length.should == 2
    Bookmark.find_all_by_session_id(request.session_options[:id]).map { |x| x.document_id }.should include("77826928")
    get :index
  ...
  it "should delete an item from list" do
    ...
    Bookmark.find_all_by_session_id(request.session_options[:id]).length.should == 1
    Bookmark.find_all_by_session_id(request.session_options[:id]).map { |x| x.document_id }.should_not include("77826928")
  end
  it "should clear list" do
    get :update, :id =>"77826928"
    get :update, :id => "94120425"
    get :clear
    Bookmark.find_all_by_session_id(request.session_options[:id]).length.should == 0
  end
